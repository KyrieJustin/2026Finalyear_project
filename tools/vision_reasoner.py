...
class VisionReasonerModel(BaseVisionModel, DetectionModel, QAModel):
    def __init__(self, reasoning_model_path="/root/autodl-tmp/huggingface/hub/Qwen2.5-VL-7B-Instruct/"):
        # 如果上面这个路径不对，请改为 "Qwen/Qwen2.5-VL-7B-Instruct" 让它自动下载
        
        self.resize_size = 840
        print(f"Loading Vision Reasoner from: {reasoning_model_path} ...")
        ...
        self.reasoning_model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
            ...
            attn_implementation="flash_attention_2" # 如果装了flash-attn就加上这句加速
        )
    ...
    def _generate_model_output(self, images, instructions, template, batch_mode=False):
        ...
        inputs = inputs.to(self.reasoning_model.device) # --- 修正: 使用模型所在的设备 ---
    ...
    # ... 后面的 DetectionModel 和 QAModel 方法保持不变 ...
    def process_single_image(self, image, instruction, return_task_type=False):
        # 注意：这里需要 task_router，但你可能没有传进来。
        # 如果报错，建议暂时写死或者简单判断
        task_type = "detection" # 临时修复：假设都是检测任务
